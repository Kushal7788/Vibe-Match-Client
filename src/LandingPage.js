    ...
    if (service === 'Netflix') {
      ...
      if (Array.isArray(proofData) && proofData.length > 0) {
        ...
        if (publicData && publicData.wigetsData) {
          const watchHistoryWidget = publicData.wigetsData.find(widget => widget.widgetType === "watch-history");
          if (watchHistoryWidget && watchHistoryWidget.content && watchHistoryWidget.content.content) {
        ...
        onSuccessCallback: async (proof) => {
          ...
          try {
            ...
            const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/data`, {
              ...
              body: JSON.stringify({ titles: titles, serviceType: service }), // Include extracted titles
            });
    ...
    if (selectedService) {
      return (
        <div className="qr-code-container">
          <h2 className="verification-headline">Link your {selectedService} data</h2>
          {isLoading ? (
            <div className="loading-scanner"></div>
          ) : (
            qrCodeUrl && <QRCode value={qrCodeUrl} size={256} />
          )}
          <p>{isLoading ? "Tuning into your vibe..." : "Scan this QR code to share your watch history"}</p>
    ...
    return (
      <>
        ...
        <div className="reclaim-buttons">
          <button className="reclaim-button netflix" onClick={() => getVerificationReq('Netflix')}>
            Netflix
          </button>
          <button className="reclaim-button amazon-prime" onClick={() => getVerificationReq('Amazon Prime')}>
            Amazon Prime
          </button>
        </div>
    ...
    <div className="vibe-matcher">
      {user && (
        <div className="user-info">
          ...
          <button className="signout-icon" onClick={handleSignOut} title="Sign Out">
            <SignOutIcon />
          </button>
        </div>
      )}
      <div className="hero-container">
        ...
        {user && (
          <div className="crazy-background">
            {createParticles(20)}
          </div>
        )}
        <div className={getContentStyle()}>
          <h1 className={`logo pixel-art ${user ? 'logged-in' : ''}`}>VIBE MATCH</h1>
          {user ? (
            <div className="logged-in-content">
              {renderContent()}
            </div>
          ) : (
            <button className="login-button pixel-art" onClick={signInWithGoogle}>
              <GoogleLogo />
